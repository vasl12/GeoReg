...
def measure_seg_metrics(metric_data):
    ...
    seg_gt = metric_data['fixed_seg'].astype('int')
    seg_pred = metric_data['warped_moving_seg'].astype('int')
    ...
    # results_tensor = torch.tensor(list(results.values()))
    # results['mean_dice'] = torch.mean(results_tensor).item().cpu()
    return results
